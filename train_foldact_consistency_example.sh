...
export TRAIN_DATA_DIR="${TRAIN_DATA_DIR:-/path/to/train/data}"
export TEST_DATA_DIR="${TEST_DATA_DIR:-/path/to/test/data}"

# WandB Configuration
WAND_PROJECT='search-agent-Context-Monitoring'

# Ray Configuration
# Use user-writable directory for Ray temporary files
RAY_TMPDIR="${RAY_TMPDIR:-/tmp/ray_tmp}"
export RAY_TMPDIR
...
# GPU Compute Mode
################################################################################
# Set GPU_IDS environment variable to specify GPU IDs (e.g., export GPU_IDS="0,1")
if [ -n "$GPU_IDS" ]; then
    nvidia-smi -i "$GPU_IDS" -c EXCLUSIVE_PROCESS
fi
...
PYTHONUNBUFFERED=1 python3 -m verl.trainer.main_ppo \
    ...
    `# Set RETRIEVER_URL environment variable (e.g., export RETRIEVER_URL="http://localhost:5000/retrieve")` \
    +retriever.url="${RETRIEVER_URL:-http://localhost:5000/retrieve}" \
